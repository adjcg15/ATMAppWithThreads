...
class Program
{
    ...
    static void PerformTransaction(object? threadId) {
        for (int i = 0; i < 5; i++)
        {
            int amuntToWithdraw = random.Next(10, 101);

            Thread.Sleep(1000);

            lock(typeof(Program)) {
                if(accountBalance >= amuntToWithdraw) {
                    accountBalance -= amuntToWithdraw;
                    Console.WriteLine($"Thread {threadId}: Se retiraron ${amuntToWithdraw} pesos. Quedan ${accountBalance} pesos");
                }
                else 
                {
                    Console.WriteLine($"Thread {threadId}: Fondos insuficientes. Se requieren: ${amuntToWithdraw} pesos.");
                }
            }
        }
    }
